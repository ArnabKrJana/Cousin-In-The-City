...
**Answer:** "This was a classic LLM prompt confusion issue, especially prevalent in smaller, highly quantized local models like `llama3.2:3b`. The Orchestrator's System Prompt instructed the AI to 'append a JSON block' to trigger Android hardware intents. However, because Spring AI's internal tool-calling architecture *also* relies on JSON blocks under the hood, the model became confused when asked to perform a physical action (like 'add to calendar'). Instead of generating the requested Intent JSON, it hallucinated and leaked its internal tool-calling parameters into the chat UI.
To fix this, I engineered a highly explicit prompt boundary in the backend's `AiConfig.kt`. I provided exact JSON templates for `CALENDAR`, `MAP`, and `KEEP` intents, and explicitly commanded the model: `CRITICAL: NEVER output {"name": "functionName"} JSON blocks to the user. Only output the {"intent": "..."} block when requested.` This strict negative constraint resolved the hallucination and ensured the Android UI reliably intercepted valid Intent triggers."
