...
### Q14: Are your domain modules (mcp-travel, mcp-finance) true Microservices? What problems do Microservices solve, what problems do they create, and how did you work around them?
**Answer:** "Yes, they are 100% true Microservices. Each module (like `mcp-travel`) is a completely independent Spring Boot application with its own embedded Tomcat server, running on its own dedicated port (e.g., 8081). 
- **The Problem They Solve (Scalability & Fault Isolation):** In a traditional Monolith, if the travel API crashes due to an out-of-memory error, the entire application goes down. By using Microservices, we achieve fault isolation. If `mcp-travel` crashes, the `agent-orchestrator` on port 8080 stays perfectly alive. Additionally, if flight search traffic spikes, I can deploy 10 instances of `mcp-travel` without having to scale the other services.
- **The Problem They Bring (Network Latency & Fragility):** Instead of calling a Java method in the same memory space, the Orchestrator now has to make a physical network request (HTTP GET) to the travel service. Networks are inherently unreliable—packets drop, connections timeout, and latencies spike.
- **The Workaround:** To solve the fragility, I implemented **Resilience4j Circuit Breakers**, which monitor the network and gracefully cut off traffic to failing microservices before they can freeze the Orchestrator. To solve the network latency and prevent DDOSing my own services, I implemented a **Redis Distributed Cache** directly on the HTTP clients (`@Cacheable`), ensuring that duplicate AI requests are served instantly from RAM without ever hitting the network."

### Q15: What is MCP (Model Context Protocol)? Are your servers true MCP servers?
**Answer:** "The Model Context Protocol (MCP) is an open-source standard created by Anthropic. It standardizes how AI models connect to external data sources and tools. A technically 'true' MCP server implements Anthropic's exact JSON-RPC wire protocol, usually communicating over Standard Input/Output (stdio) or Server-Sent Events (SSE).
...
### Q19: How did you bridge the gap between unstructured LLM outputs and native mobile OS APIs (like Android Intents)?
**Answer:** "Native mobile APIs, such as Android's `CalendarContract`, require strictly typed data—like Epoch milliseconds for `EXTRA_EVENT_BEGIN_TIME`. LLMs naturally output unstructured conversational text. To bridge this, I engineered a strict JSON structured output schema (`AgentResponse`) containing an `intentType` and an `actionData` map. I then injected explicit constraints into the Orchestrator's System Prompt, forcing the LLM to format dates as `yyyy-MM-dd` and times as `h:mm a` specifically for the `CALENDAR` intent, and separating titles from bodies for the `KEEP` intent. On the Android side, I intercepted this JSON payload, parsed the standardized strings into a `LocalDateTime`, converted it to epoch milliseconds, and securely launched the native `ACTION_INSERT` Android Intent. This architecture seamlessly translated natural language requests into deterministic OS-level actions without relying on heavy third-party parsing libraries."

### Q20: How did you ensure Separation of Concerns and prevent data leakage in your Spring Boot REST APIs?
**Answer:** "I enforced strict Enterprise Clean Architecture by creating a hard boundary between my database layer and my network layer. Specifically, I never allowed JPA Entities (like `AppUser` or `ChatThread`) to be returned directly from the `@RestController`. Instead, I decoupled the layers by creating a dedicated `@Service` layer to handle business logic, mapping all repository queries into dedicated Network DTOs (`AppUserDto`, `ChatThreadDto`) using Kotlin Extension Functions before returning them to the Controller. This pattern securely hides the underlying PostgreSQL schema from the Android client and ensures that future database refactors will never inadvertently break the mobile Retrofit API contracts."
